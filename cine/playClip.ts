...
import { PlayClipOptions, ToolData, CinePlayContext } from './CINETypes';
 ...
 */
function playClip(element: HTMLDivElement, playClipOptions: PlayClipOptions): void {
  ...
  const playClipAction = () => {
    ...
    if (
      ...
      const delta = playClipOptions.frameRangeStart - currentStepIndex;
      if (delta) {
        playClipContext.scroll(delta);
      }
    }
...
function _stopClip(
  ...
  if (stopDynamicCine && enabledElement?.viewport instanceof BaseVolumeViewport) {
    _stopDynamicVolumeCine(element);
  }
}
...
function _createVolumeViewportCinePlayContext(
  ...
  return {
    ...
    get frameTimeVectorEnabled(): boolean {
      const camera = viewport.getCamera();
      const volumeViewPlaneNormal = volume.direction.slice(6, 9).map(x => -x) as Types.Point3;
...
function _createCinePlayContext(viewport, playClipOptions: PlayClipOptions): CinePlayContext {
  ...
  if (viewport instanceof VolumeViewport) {
    ...
    if (playClipOptions.dynamicCineEnabled && volume?.isDynamicVolume()) {
      return _createDynamicVolumeViewportCinePlayContext(<Types.IDynamicImageVolume>volume);
    }
